rankhazard_CI.cph <- function(cphobj,  x, refpoints, CI_level){
  ...
  Values <- rankhazard_CI.default(modelobj = cphobj, x = x, coefs = coefs, refpoints = refpoints, 
                                  factorlevels = factorlevels, covariatelabs = covariatelabs)
  CIlow <- rankhazard_CI.default(modelobj = cphobj, x = x, coefs = coefs_low, refpoints = refpoints, 
                                 factorlevels = factorlevels, covariatelabs = covariatelabs)
  CIupp <- rankhazard_CI.default(modelobj = cphobj, x = x, coefs = coefs_upp, refpoints = refpoints, 
                                 factorlevels = factorlevels, covariatelabs = covariatelabs)
